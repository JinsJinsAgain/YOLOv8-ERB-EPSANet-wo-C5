...
from ultralytics.nn.modules import (AIFI, C1, C2, C3, C3TR, SPP, SPPF, EMRP, Bottleneck, BottleneckCSP, C2f, C3Ghost, C3x,
                                    ...
                                    Segment, Bottleneck_eff, ERB, EfficientSpatialAttention, SERA, ConvNeXt, LayerNorm, C2Conv, SEWeightModule, PSAModule, EPSABlock)
...
class SegmentationModel(DetectionModel):
    ...
    def _forward_augment(self, x):
        ...
        raise NotImplementedError(emojis('WARNING SegmentationModel has not supported augment inference yet!'))
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in (Classify, Conv, ConvTranspose, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, EMRP, DWConv, Focus,
                 BottleneckCSP, C1, C2, C2f, C3, C3TR, C3Ghost, nn.ConvTranspose2d, DWConvTranspose2d, C3x, RepC3, Bottleneck_eff, ERB, EfficientSpatialAttention, SERA, C2Conv):
            c1, c2 = ch[f], args[0]
            if c2 != nc:  # if c2 not equal to number of classes (i.e. for Classify() output)
                c2 = make_divisible(min(c2, max_channels) * width, 8)

            args = [c1, c2, *args[1:]]
            if m in (BottleneckCSP, C1, C2, C2f, C3, C3TR, C3Ghost, C3x, RepC3, Bottleneck_eff, ERB, C2Conv):
